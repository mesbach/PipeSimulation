...
{
    class PipaAngsurBesar : DuaPipa
    {
        public Double SudutPengecilan { get; set; }
        public PipaAngsurBesar()
        {
            
        }
        protected override double setKehilanganEnergi()
        {
            this.KehilanganEnergi = getKoefisien() * (Math.Pow(this.KecepatanAliran, 2) - Math.Pow(this.KecepatanAliran2, 2)) / (2 * c.Gravity);
